...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def extract_feat(self, img, dynamic_input=None):
        if dynamic_input is None:
            x = self.backbone(img)
        else:
            x, inp_atten = self.backbone(img)
        if self.with_neck:
            x = self.neck(x)
        if dynamic_input is None:
            return x
    ...
    def forward_train_dct(self, dct_y, dct_cb, dct_cr, img_meta, gt_bboxes, gt_labels,
                          gt_bboxes_ignore=None, gt_masks=None, proposals=None, dynamic_input=None):
        if dynamic_input is None:
    ...
    def forward_train(self,
        ...
        if dynamic_input is None:
            ...
            x, gate_activations = self.extract_feat(img, dynamic_input)
        ...
        if dynamic_input is not None:
            ...
            for ga in gate_activations:
                acts += torch.mean(ga)
            loss_gate_activations = acts / len(gate_activations) * dynamic_input[0]
            losses.update(dict(loss_gate_activations=loss_gate_activations))

        return losses

    def simple_test(self, img, img_meta, img_raw=None, proposals=None, rescale=False, dynamic_input=False):
        """Test without augmentation."""
        assert self.with_bbox, "Bbox head must be implemented."
        dynamic_input = True if dynamic_input is not False else False

        if not dynamic_input:
            x = self.extract_feat(img)
        else:
            x, gate_activations = self.extract_feat(img, dynamic_input=dynamic_input)
        ...
        if not self.with_mask:
            ...
            if not dynamic_input:
                ...
                return bbox_results, segm_results, gate_activations
    ...
    def simple_test_dct(self, dct_y, dct_cb, dct_cr, img_meta, proposals=None, rescale=False):
        """Test without augmentation."""
        assert self.with_bbox, "Bbox head must be implemented."
    ...
    def aug_test_dct(self, dct_y, dct_cb, dct_cr, img_metas, rescale=False):
        ...
        # recompute feats to save memory
        proposal_list = self.aug_test_rpn(
            self.extract_feats_dct(dct_y, dct_cb, dct_cr), img_metas, self.test_cfg.rpn)
        det_bboxes, det_labels = self.aug_test_bboxes(
            self.extract_feats_dct(dct_y, dct_cb, dct_cr), img_metas, proposal_list, self.test_cfg.rcnn)
        ...
        if self.with_mask:
            segm_results = self.aug_test_mask(
                self.extract_feats_dct(dct_y, dct_cb, dct_cr), img_metas, det_bboxes, det_labels)
            return bbox_results, segm_results
        else:
            return bbox_results